...
document.getElementById('btnLogout').onclick = () => {
    if (confirm("¿Cerrar sesión?")) {
        localStorage.clear();
        window.location.replace('login.html'); // Replace evita volver atrás
    }
};
...
async function cargarDatosAdmin() {
    const h = getHeaders();
    const resU = await fetch(`${API_URL}/users/`, { headers: h });
    ...
    const resS = await fetch(`${API_URL}/sedes/`, { headers: h });
...
window.guardarSede = async () => {
    ...
    const url = id ? `${API_URL}/sedes/${id}/` : `${API_URL}/sedes/`;
...
window.guardarProceso = async () => {
    const body = { nombre: document.getElementById('proc_nombre').value, criticidad: document.getElementById('proc_crit').value, rto: parseInt(document.getElementById('proc_rto').value), rpo: parseInt(document.getElementById('proc_rpo').value), sede: parseInt(document.getElementById('sel_sede_proceso').value) };
    await fetch(`${API_URL}/procesos/`, { method: 'POST', headers: getHeaders(), body: JSON.stringify(body) });
    alert("✅ Éxito"); cargarSedes(); cargarProcesosSede();
};

window.borrarSede = async (id) => { if (confirm("¿Borrar?")) { await fetch(`${API_URL}/sedes/${id}/`, { method: 'DELETE', headers: getHeaders() }); cargarDatosAdmin(); cargarSedes(); } };
window.borrarUsuario = async (id) => { if (confirm("¿Borrar?")) { await fetch(`${API_URL}/users/${id}/`, { method: 'DELETE', headers: getHeaders() }); cargarDatosAdmin(); } };
window.borrarProceso = async (id) => { if (confirm("¿Borrar?")) { await fetch(`${API_URL}/procesos/${id}/`, { method: 'DELETE', headers: getHeaders() }); cargarSedes(); cargarProcesosSede(); } };
